...
function barber_remove_wp_version_strings( $src ){
...
add_filter( 'script_loader_src', 'barber_remove_wp_version_strings' ); //First value - filter that we whant to call (it calls all the scripts that we whant to call in the footer). Second value - function name.

add_filter( 'style_loader_src', 'barber_remove_wp_version_strings' );

/* Remove metatag generator from header */
//REMOVES WORDPRESS VERSION (for safety reasons) FROM HEADER
function barber_remove_meta_version(){
	return '';
}
